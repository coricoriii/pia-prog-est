...
void modificar_boxeador() {
    ...
    if (pos >= 0) {
        ...
        if (f != NULL) {
            ...
            printf("Nuevo peso (%.2f): ", b.peso);
            b.peso = pedir_peso();

            printf("Nuevas victorias: ");
            b.record.victorias = pedir_entero_positivo("victorias");

            printf("Nuevas derrotas: ");
            b.record.derrotas = pedir_entero_positivo("derrotas");
...
void registrar_resultado() {
    ...
    if (id_ganador == id_perdedor) {
        printf("Error: el ganador y perdedor no pueden ser el mismo boxeador.\n");
        return;
    }
